    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 text-gray-900 dark:text-gray-100">
                ...
                <form action="{{ route('loantransactions.update', $loantransaction->id) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        ...
                        <select name="customer_id" class="block w-full mt-1 border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md shadow-sm">
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ old('customer_id', $loantransaction->customer_id) == $customer->id ? 'selected' : '' }}>
                                    {{ $customer->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="loan_id" class="block w-full mt-1 border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md shadow-sm">
                            ...
                            @foreach($loans as $loan)
                                <option value="{{ $loan->id }}" {{ old('loan_id', $loantransaction->loan_id) == $loan->id ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.01" name="amount_paid" value="{{ old('amount_paid', $loantransaction->amount_paid) }}" class="block w-full mt-1 border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md shadow-sm">
                        ...
                        <div>
                            ...
                            <input type="date" name="date_paid" value="{{ old('date_paid', $loantransaction->date_paid) }}" class="block w-full mt-1 border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md shadow-sm">
